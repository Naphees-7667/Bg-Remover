      ...
      <div className="flex flex-wrap justify-center gap-6 text-left">
        {plans.map((item, index) => (
          ...
          >
            ...
            <p className="m-3 font-semibold">{item.id}</p>
            <p className="text-sm">{item.desc}</p>
            <p className="mt-6">
              <span className="text-3xl font-medium">${item.price}</span>/<span>{item.credits}</span> credits 
            </p>
            <button onClick={() => paymentRazorpay(item.id)} className="w-full bg-gray-800 text-white py-2.5 mt-8 text-sm rounded-full min-w-52">Buy Now</button>
          </div>
        ))}
      </div>
    </div>
